...
def process(message, encodeType):

    _replayMTI(message['t'])

    transactionCode = message['3']

    if transactionCode == '360000':
        banksumut._inquery(message)
    elif transactionCode == '560000':
        banksumut._payment(message)

    encoded_raw, encoded = iso8583.encode(banksumut.response['bitmap'], sumutSpec)

    if encodeType == 'raw':
        return encoded_raw.decode('utf-8')

    return encoded

def _inquery(message):
    banksumut._merchant(message)

    bitmapAdjusment = {"t", "39", "48", "62"}
    
    for key in banksumut.response['bitmap']:
        if key not in bitmapAdjusment:
            banksumut.response['bitmap'][key] = message[key]

    # Response Code
    banksumut.response['bitmap']['39'] = "00"

    # Static additional Data
    banksumut.response['bitmap']['48'] = '1209411011001222161  120912001701800920       FERDAWATY BR KARO             TIBAN HOUSING A1/4                           411011001SSPD BPHTB                                        2022      01222161            000003500000000000000000000000002000'
    
    # Static data Pemda
    banksumut.response['bitmap']['62'] = 'PEMKAB KARO                   '

    return banksumut.response

def _payment(message):
    return {}

def _reversal(message):
    return {}

def _replayMTI(mti):
    if mti == '0200':
        banksumut.response['bitmap']['t'] = '0210'
    elif mti == '0400':
        banksumut.response['bitmap']['t'] = '0410'
    elif mti == '0401':
        banksumut.response['bitmap']['t'] = '0411'

def _merchant(message):
    processCode = message['18']    

    if processCode == '7010':
        banksumut.response['adminFee'] = 2000
        banksumut.response['merchantName'] = 'atm'
    elif processCode == '7012':
        banksumut.response['adminFee'] = 0
        banksumut.response['merchantName'] = 'teller'
    elif processCode == '7016':
        banksumut.response['adminFee'] = 2000
        banksumut.response['merchantName'] = 'mobile'
